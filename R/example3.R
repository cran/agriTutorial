...
#' greenrice.uptake = lmer(uptake ~ Replicate + factor(N) * factor(W) + (1|Replicate:Main),
#'  data = greenrice)
...
#'  main = "Fig 4a: Marginal model for water stress",
#'  xlab = " Water stress (days)", ylab = "Log nitrogen uptake (g/pot)",
#'  strip = strip.custom(strip.names = TRUE,
#'  factor.levels = c("0", "90", "180", "270")),
#' panel = panel.plot)
#'
#' ## Fig 4b fitted quadratic loguptake curve versus nitrogen rate treatments
#' panel.plot = function(x, y) {
#' panel.xyplot(x, y) # shows observed points
...
#'  main = "Fig 4b: Marginal model for nitrogen rate",
